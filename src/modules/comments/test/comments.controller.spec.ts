...
describe('CommentsController', () => {
	...
	describe('findAll', () => {
		it('should be return comments data based on filter', async () => {
			...
			// Act
			await comments_controller.findAll(
				filter.target_id,
			...
			// Assert
			expect(comments_service.findAll).toBeCalledWith(filter, {
				...get_all_options,
				including_children,
			});
		});
	...
	describe('getAllSubComments', () => {
		it('should be return all reply of specific comment', async () => {
			// Arrange
			const filter = {
			...
			const deep_level = 2;
			// Act
			await comments_controller.getAllSubComments(
				filter.comment_id,
				filter.target_id,
				deep_level,
			);
			...
			expect(comments_service.getAllSubComments).toBeCalledWith(
				{ parent_id: filter.comment_id, target_id: filter.target_id },
				deep_level,
			);
		});
	});
